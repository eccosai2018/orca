...
function probeNotificationDelivery(): Promise<NotificationDeliveryProbeResult> {
  ...
  const probe = new Notification({
    ...
    body: 'Dark Factory will alert you when agents finish or terminals need attention.',
    silent: true
  })
...
export function triggerStartupNotificationRegistration(store: Store): void {
  ...
  const notification = new Notification({
    title: 'Dark Factory is ready to notify you',
    body: 'Allow notifications so Dark Factory can alert you when agents finish or terminals need attention.'
  })
